...
- **Not a general-purpose scraper.** The `.cl`-only email filter
  ([ADR-0003](docs/adr/0003-crawl-phase1-phase2-scope.md)) is a permanent
...
- **Not committing to a cross-platform GUI.** PySide6/Qt6
  ([ADR-0001](docs/adr/0001-pyside6-gui-stack.md)) stays a Windows-first
...
- Must run acceptably on low-spec hardware (old office desktops), per
  [ADR-0005](docs/adr/0005-hardware-aware-auto-tuning.md) — any new
...
- Session data is treated as sensitive and non-permanent by design
  ([ADR-0006](docs/adr/0006-sqlite-session-store-last-10-pruning.md)) —
